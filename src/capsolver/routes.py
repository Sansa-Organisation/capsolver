...
@router.get("/health")
async def health():
    return {"status": "ok", "service": "capsolver", "version": "0.3.8", "supported_types": SUPPORTED_TYPES}
...
async def solver_info():
    """Explain all captcha types and solver strategies v0.3.8."""
    return {
        "version": "0.3.8",
        "supported_types": SUPPORTED_TYPES,
        "aliases": TYPE_ALIASES,
...
async def solve_challenge(session_id: str, max_retries: int = 5, captcha_type: Optional[str] = None, sweep: bool = True):
    """Attempt to solve captcha in session with trusted drag - v0.3.8 broad sweep T001 proven."""
