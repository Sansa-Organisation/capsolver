...
@router.get("/health")
async def health():
    return {"status": "ok", "service": "capsolver", "version": "0.3.3", "supported_types": SUPPORTED_TYPES}
...
async def solver_info():
    ...
    return {
        ...
        "version_notes": "v0.3.3 robust slider selector: 15+ selectors #aliyunCaptcha-sliding-slider etc + fallback cap container size 30-70px + selector-based trusted drag via OxyBlink, fixing 0,0 coord bug. v0.3.2 trusted CDP drag isTrusted=true F015->F000. v0.3.0 added RECAPTCHA etc.",
    }
...
async def solve_challenge(session_id: str, max_retries: int = 3, captcha_type: Optional[str] = None):
    """Attempt to solve captcha in session with trusted drag."""
    success, param, info = browser.solve_captcha_in_session(session_id, max_retries=max_retries)
    return {
        "success": success,
